 ...
 * @version: 1.0.0
 */
public class EnumCheckValidateNotNull implements ConstraintValidator<EnumCheckNotNull, Object> {
    /**
     * @description: 最小值
     */
    private Integer min;
    ...
    @Override
    public void initialize(EnumCheckNotNull enumCheckNotNull) {
        String field = enumCheckNotNull.field();
        Class<? extends Enum<?>> enumCLass = enumCheckNotNull.target();
        min = enumCheckNotNull.min();
        max = enumCheckNotNull.max();
        checkRange = enumCheckNotNull.checkRange();
        // 初始化容器对象
    ...
    public boolean isValid(Object value, ConstraintValidatorContext constraintValidatorContext) {
        // 传入值为空,禁用原始文本提示信息并构建新的提示信息
        if (null == value) {
            constraintValidatorContext.disableDefaultConstraintViolation();
            constraintValidatorContext.buildConstraintViolationWithTemplate("请传入用户性别").addConstraintViolation();
            return false;
        }
